...
- Work as one vertical slice, red → green: call the Skill tool with "mattpocock-skills:tdd" and follow its loop, one failing test then the minimal code, seam by seam. When the interface shape itself is in question, call the Skill tool with "mattpocock-skills:codebase-design".
